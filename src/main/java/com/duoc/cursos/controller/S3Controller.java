...
public class S3Controller {
    ...
    public ResponseEntity<Resource> descargarArchivoS3(@PathVariable Long id) {
        try {
            ...
            // 2️⃣ Fix para el segundo Warning: Usar el objeto estático directo en contentType
            return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"resumen_" + id + ".txt\"")
                    .contentType(MediaType.TEXT_PLAIN) // VS Code a veces prefiere .contentType(MediaType.parseMediaType("text/plain")) si alega por la anotación @NonNull
                    .body(resource);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        }
